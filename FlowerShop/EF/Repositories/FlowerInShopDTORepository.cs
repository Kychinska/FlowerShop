        ...
        public List<FlowerInShopDTO> GetAvailableFlowers()
        {
            ...
            {                             
                return ctx.FlowerPrices.Join(ctx.Flowers, fp => fp.FlowerId, f => f.Id, (p, d) => new FlowerInShopDTO() 
                                                        {FlowerId=d.Id,FlowerName=d.Name,FlowerPrice=p.PriceFlower,Color=d.Color,FlowerNumber=p.Number}).ToList();
            }
        }
        public List<FlowerInShopDTO> GetNotAvailableFlowers()
        {
            using (var ctx = new FlowerShopContext())
            {
                return ctx.FlowerPrices.Join(ctx.Flowers, fp => fp.FlowerId, f => f.Id, (p, d) => new FlowerInShopDTO()
                { FlowerId = d.Id, FlowerName = d.Name, FlowerPrice = p.PriceFlower, Color = d.Color,
                    FlowerNumber = p.Number-ctx.Bouquets.Where(b=>DbFunctions.DiffMinutes(DateTime.Now,b.DateTimeOfCreateBouquet)<20)
